...
export class P2pSource extends EventEmitter {
  constructor(camera, { client, ffmpeg = 'ffmpeg', noDataMs = 15_000 } = {}) {
    super();
    this.camera = camera;
    this._client = client;
    this._ffmpeg = ffmpeg;
  ...
  async start() {
    ...
    proc.stdout.on('data', (buf) => {
      if (!this._gotData) {
        ...
        log(`p2p:${this.id}`, `eerste videodata (${buf.length} bytes) — streaming`);
    ...
    proc.on('error', (err) => {
      this._proc = null;
      logErr(`p2p:${this.id}`, 'ffmpeg error', err.message);
      this.emit('error', err);
    });

    this._onStream = (serial, videoStream) => {
      if (serial !== this.camera.serial || !this._proc) {
        return;
      }
    ...
    this._watchdog = setTimeout(() => {
      if (!this._gotData) {
        logErr(
          `p2p:${this.id}`,
          `geen videodata binnen ${this._noDataMs / 1000}s — camera verbond wel maar levert geen beeld (waarschijnlijk de mei-2026 P2P-bug). Probeer RTSP voor deze camera.`,
        );
    ...
    try {
      ...
      logErr(`p2p:${this.id}`, 'startLivestream faalde', error.message);
  ...
  async stop() {
    ...
    this._gotData = false;
    if (this._onStream) {
      this._client.off('livestream', this._onStream);
      this._onStream = null;
    }
    try {
      await this._client.stopLivestream(this.camera.serial);
    } catch {}
    if (this._proc) {
      const p = this._proc;
      this._proc = null;
