...
class CameraService {
  constructor() {
    ...
    this.makeSource = (cam) =>
      cam.type === 'p2p' ? new P2pSource(cam, { client: eufyClient }) : new RtspSource(cam);
  ...
  async refresh() {
    if (this.refreshing || !eufyClient.isConnected()) {
      return;
    }
    this.refreshing = true;
    try {
      const next = await eufyClient.getCameras();
      if (next.length !== this.cameras.length) {
        log('eufy', `${next.length} camera('s) gevonden`);
      }
      this.cameras = next;
    } catch {
      /* volgende keer weer */
  ...
  status() {
    return {
      configured: eufyConfigured() || eufyClient.isConnected(),
      connected: eufyClient.isConnected(),
    };
  }
}
