...
export const createVideoSocket = (service) => {
  ...
  const sendToChannel = (id, payload) => {
    for (const ws of channels.get(id) || []) {
      if (ws.readyState === 1) {
        ws.send(payload);
      }
    }
  ...
  manager.on('camera-error', (id, err) =>
    sendToChannel(id, JSON.stringify({ event: 'error', message: String(err?.message || err) })),
  );

  wss.on('connection', (ws, camera) => {
    const viewerId = `ws${++counter}`;
    if (!channels.has(camera.id)) {
      channels.set(camera.id, new Set());
    }
    channels.get(camera.id).add(ws);
    log('socket', `kijker erbij op ${camera.id} (${camera.type})`);
    manager.addViewer(camera, viewerId);
    ws.on('close', () => {
      channels.get(camera.id)?.delete(ws);
      manager.removeViewer(camera.id, viewerId);
      log('socket', `kijker weg bij ${camera.id}`);
    });
    ws.on('error', () => {
      try {
        ws.close();
  ...
  const handleUpgrade = (req, socket, head) => {
    ...
    if (!match) {
      return false;
    } // niet voor ons (bijv. Next hot-reload)
    const camera = service.getCamera(decodeURIComponent(match[1]));
    // No such camera, or its RTSP URL isn't ready yet → accept the socket then close it
    // cleanly with a retryable code, so the client retries shortly instead of getting an
    // abrupt destroy it can't tell apart from a crash.
    if (!camera || (camera.type === 'rtsp' && !camera.url)) {
      wss.handleUpgrade(req, socket, head, (ws) => ws.close(4002, 'camera nog niet klaar'));
      return true;
    }
