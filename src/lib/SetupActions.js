...
const finishIfConnected = async (result) => {
  if (result.status === 'connected') {
    ...
    try {
      result.cameras = await eufyClient.getCameras();
    } catch {
      result.cameras = [];
    }
  }
...
export const getState = async () => {
  ...
  if (eufyClient.isConnected()) {
    try {
      cameras = await eufyClient.getCameras();
    } catch {
      cameras = [];
    }
  }
  return {
    cameras,
    eufy: { configured: eufyClient.configured(), connected: eufyClient.isConnected() },
  };
};
...
const startConnect = async (account) => {
  if (eufyClient.isConnected()) {
    return finishIfConnected({ status: 'connected' });
  }
  lastAccount = account;
  const step = nextStep();
  eufyClient
    .connectWith(account, {
      captchaHandler: (image, id) => {
        reportStep({ status: 'captcha', captchaId: id, image });
        return new Promise((r) => captchaAnswers.set(id, r));
      },
      verifyHandler: () => {
        reportStep({ status: '2fa' });
        return new Promise((r) => {
          tfaAnswer = r;
        });
      },
    })
    .then(() => reportStep({ status: 'connected' }))
    .catch((error) => reportStep({ status: 'error', message: String(error?.message || error) }));
  return finishIfConnected(await step);
};

export const eufyConnect = async ({ username, password, country }) => {
  if (!username || !password || !country) {
    throw new Error('gebruikersnaam, wachtwoord en land zijn verplicht');
  }
...
export const eufyCaptcha = async ({ captchaId, code }) => {
  ...
  if (!resolver) {
    throw new Error('geen captcha in behandeling');
  }
...
export const eufyTfa = async ({ code }) => {
  if (!tfaAnswer) {
    throw new Error('geen 2FA in behandeling');
  }
  const resolver = tfaAnswer;
  tfaAnswer = null;
  const step = nextStep();
  resolver(String(code || ''));
  return finishIfConnected(await step);
