...
export class LivePlayer {
  ...
  start(cameraId, { onLive, onError, onClose, onAutostopped, onDebug } = {}) {
    ...
    ws.onmessage = (ev) => {
      if (typeof ev.data === 'string') {
        try {
          ...
          if (m.event === 'autostopped') {
            onAutostopped?.();
          } else if (m.event === 'error') {
            onError?.(new Error(m.message || 'stream-fout'));
          }
        } catch {
          /* negeren */
        }
      ...
      try {
        this.jmuxer.feed({ video: new Uint8Array(ev.data) });
      } catch {
        /* decoder-hik */
      }
    };
    ...
    this.watch = setInterval(() => {
      if (!first && Date.now() - lastData > 10_000) {
