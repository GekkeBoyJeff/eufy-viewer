...
const remember = (line) => { buffer.push(line); if (buffer.length > 300) buffer.shift(); };
const format = (scope, args) => `${time()} [${scope}] ${args.map((a) => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')}`;

export const log = (scope, ...args) => { const line = format(scope, args); remember(line); console.log(line); };
export const logErr = (scope, ...args) => { const line = format(scope, args); remember(line); console.error(line); };
