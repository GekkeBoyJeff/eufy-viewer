...
class EufyClient extends EventEmitter {
  ...
  async _connectStation(station) {
    ...
    try { station.connect(); } catch { /* volgende ronde opnieuw */ }
    if (!canCheck) { await new Promise((r) => setTimeout(r, 2500)); return true; }
    for (let i = 0; i < 24; i += 1) {
      if (station.isConnected()) return true;
  ...
  async _resolveOnce(serial) {
    try {
      ...
      const device = await withTimeout(this._eufy.getDevice(serial), 10000);
      if (!device) { logErr('rtsp', `${serial}: camera niet gevonden — volgende ronde opnieuw`); return null; }
      ...
      const station = await withTimeout(this._eufy.getStation(serial), 10000);
      if (!station) { logErr('rtsp', `${name}: station niet gevonden — volgende ronde opnieuw`); return null; }
      if (!(await this._connectStation(station))) { logErr('rtsp', `${name}: station nog niet verbonden — volgende ronde opnieuw`); return null; }
      ...
      if (url) { this._rtspCache.set(serial, url); log('rtsp', `${name}: RTSP klaar (${maskUrl(url)})`); return url; }
      logErr('rtsp', `${name}: RTSP nog niet gelukt — volgende ronde opnieuw`);
      return null;
    } catch (e) { logErr('rtsp', `RTSP-fout (${serial}): ${e.message}`); return null; }
  }
