...
class EufyClient extends EventEmitter {
  ...
  async _ensureRtsp(serial) {
    ...
    // Harde time-out: één hangende camera mag de hele lijst nooit blokkeren. Mislukt
    // het, dan geeft deze poging op (null) en probeert de volgende ronde het opnieuw.
    const attempt = Promise.race([work.catch(() => null), new Promise((resolve) => setTimeout(() => resolve(null), 15000))]);
  ...
  async getReadout() {
    ...
    for (const d of cams) {
      ...
      out.push({
        ...
        url: this._rtspCache.get(serial) || props.rtspStreamUrl || null,
        props,
        station,
      });
    }
