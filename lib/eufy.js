...
class EufyClient extends EventEmitter {
  ...
  async connectWith(parts, handlers) {
    ...
    if (this._connecting) throw new Error('even wachten — er loopt al een verbindingspoging');
    ...
    try {
      ...
      logErr('eufy', `verbinden mislukt: ${e.message}`);
      throw e;
    } finally {
      this._connecting = false;
    }
  ...
  // LAN IP (rtsp://<ip>:554/live0). We poll both for a while and take whichever comes first.
  async _resolveRtspUrl(device, station) {
    for (let i = 0; i < 24; i += 1) {
      const reported = device.getPropertyValue?.(PropertyName.DeviceRTSPStreamUrl);
      if (reported) return reported;
      const ip = station.getPropertyValue?.(PropertyName.StationLanIpAddress)
        || station.getPropertyValue?.(PropertyName.StationLanIpAddressStandalone);
      if (ip) return `rtsp://${ip}:554/live0`;
      await new Promise((r) => setTimeout(r, 500));
    }
    return null;
  }

  // Turn on RTSP for one camera and work out its URL. This is best-effort and runs in the
  // background: if it doesn't succeed (camera offline, no LAN IP yet) it just returns null
  // and the next refresh calls it again — so a camera keeps trying instead of giving up.
  ...
  async _ensureRtsp(serial) {
    ...
    const work = (async () => {
      ...
      const reported = device.getPropertyValue?.(PropertyName.DeviceRTSPStreamUrl);
      if (reported) { this._rtspCache.set(serial, reported); log('rtsp', `${name}: RTSP klaar (${maskUrl(reported)})`); return reported; }
      ...
      if (url) { this._rtspCache.set(serial, url); log('rtsp', `${name}: RTSP klaar (${maskUrl(url)})`); }
      else logErr('rtsp', `${name}: RTSP nog niet gelukt — volgende ronde opnieuw`);
      return url;
    })();

    // Harde time-out: een hangende camera mag deze poging nooit eindeloos open laten staan,
    // anders blokkeert het de volgende retry. Mislukt het, dan probeert de volgende ronde opnieuw.
    const attempt = Promise.race([
      work.catch((e) => { logErr('rtsp', `RTSP-fout (${serial}): ${e.message}`); return null; }),
  ...
  async getCameras() {
    ...
    return cams.map((d) => {
      ...
      if (this._deviceSupportsRtsp(d)) {
        ...
        if (!url) this._ensureRtsp(serial).catch(() => {});
        return { id: `eufy_${serial}`, name, model, type: 'rtsp', url, battery: false, transcode: false, serial, ready: !!url };
      }
      // Genuinely RTSP-incapable cameras (e.g. battery SoloCams) use P2P.
      return { id: `eufy_${serial}`, name, model, type: 'p2p', serial, battery: typeof d.hasBattery === 'function' ? d.hasBattery() : true, transcode: false, ready: true };
    });
  }
  ...
  async getReadout() {
    ...
    for (const d of cams) {
      ...
      let station = {};
      try { const st = await this._eufy.getStation(serial); station = (st.getProperties && st.getProperties()) || {}; } catch { /* geen station-info */ }
