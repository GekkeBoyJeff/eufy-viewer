...
const slug = (name) => String(name).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'cam';
...
export const addRtspCamera = ({ name, url, transcode }) => {
...
export const deleteCamera = (id) => ({ cameras: removeCamera(id) });

export const testRtsp = async ({ url, transcode }) => {
...
export const streamTest = async ({ url }) => {
  if (!url) throw new Error('url ontbreekt');
