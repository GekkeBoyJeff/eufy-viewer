...
export function loadCameras(data) {
...
export function loadCamerasFile(file = path.join(ROOT, 'cameras.json')) {
  if (!fs.existsSync(file)) return [];
  return loadCameras(JSON.parse(fs.readFileSync(file, 'utf8')));
}

// Fill in the fixed defaults around the account details.
function withDefaults(username, password, country, persistentDir) {
  return {
...
export function loadEufyConfig(env = process.env) {
...
export function eufyConfigFromParts({ username, password, country, persistentDir } = {}) {
...
export function eufyConfigured(env = process.env) {
  return Boolean(env.EUFY_USERNAME && env.EUFY_PASSWORD && env.EUFY_COUNTRY);
}
