...
export const createVideoSocket = (service) => {
  ...
  const handleUpgrade = (req, socket, head) => {
    ...
    // No such camera, or its RTSP URL isn't ready yet → don't open a stream (the tile shows
    // "klaarzetten…" and reconnects once the camera becomes ready).
    if (!camera || (camera.type === 'rtsp' && !camera.url)) { socket.destroy(); return true; }
